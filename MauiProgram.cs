...
public static class MauiProgram
{
    ...
    public static async Task<MauiApp> CreateMauiApp()
    {
        ...
        str.ForEach(x =>
        {
            pk += Utilities.BinaryToString(x);
        });

        var chunks = Utilities.Split(pk, 5).ToList();

        var hs = new HardwareService();
        var l2 = new List<string>();
        hs.GenerateWords(chunks).ForEach(x =>
        {
            l2.Add(x.Name);
        ...
        var pk2 = string.Empty;
        l2.ForEach(x =>
        {
            pk2 += Utilities.BinaryToString(x);
        });

        var acc2 = new Account(pk2, 56);
        var test = pk == pk2;
